#!/usr/bin/env python3
"""
Vereinfachte Integrationstests für das Flask-Backend mit pytest
Fokus auf die Hauptfunktionalitäten mit gemockter Datenbank
"""
import pytest
import json
...
class TestFlaskBackendIntegration:
    ...
    def test_multiple_notes_retrieval(self, client, auth_headers, mock_user):
        """Test: Abruf mehrerer Notizen"""
        notes = []
        for i in range(3):
            note = MagicMock(spec=Note)
            note.id = i + 1
        ...
        with patch.object(SessionService, 'get_user_from_session', return_value=mock_user), \
            ...
            assert data['notes'][0]['title'] == 'Test Note 1'
            assert data['notes'][1]['title'] == 'Test Note 2'
            assert data['notes'][2]['title'] == 'Test Note 3'
